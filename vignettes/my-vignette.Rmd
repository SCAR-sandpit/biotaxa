...
`taxaaccum()` generates the accumulation curve of taxa in a lower rank belong to the same higher taxa. For instance, the following example generates the accumulation curve of discovered phyla belong to *Animalia* over the past. Note that here *taxa* needs to be in higher order than *rank*.
...
`taxamodel_cor()` yields the correlation coefficient of selected model.
...
`frequencyrank()` ranks the frequency of a lower taxa belong to the same higher taxa
...
```{r top n example}
#example: return the 5 most frequent phyla of Animalia and reports their frequencies
